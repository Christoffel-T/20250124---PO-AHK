...
class TraderBot {
    __New(settings_obj) {
        ...
        this.F300 := {
            streak7_40: {state_5lost: 0, amt: 0, sum_amt: 0, sum_over200: 0, idx: 0, losses: 0},
    ...
    AmountOverride2() {
        ...
        _helper0811_4Loss() {
            Helper0811_4Loss.Update(streak, this.streak_prev, this.max_diff.C)
            inst := Helper0811_4Loss.Get()
            this.stats.streak := inst.streak

            if this.max_diff.C <= 0 {
    ...
    CheckTradeClosed(just_check:=false) {
        ...
        HelperSumAmt(streak_obj, target_streak) {
            ...
            if (streak_prev = target_streak and streak > streak_prev) {
                streak_obj.sum_amt := Max(streak_obj.sum_amt - this.amt_prev[1], 0)
            }
        }
        ...
        HelperPause2_Loss5(streak_obj, target_streak) {
                ...
                if (streak_obj.lose_streak = 0) {
                    if (streak_obj.loss_streak_at_0 = 0) {
                        ...
                        if (this.balance.side <= 7000) {
                            num := streak_obj.perc_107
                        } else if (this.balance.side <= 10000) {
                            num := 113
                        }
                        streak_obj.next_bet_at_0 := streak_obj.last_bet_at_0 * num/100 + 1
                    }
            ...
            num_target := 5
            if (streak_obj.lose_streak >= num_target) {
        ...
        Helper1_StrPrev(streak_obj, target_streak) {
            ...
            if (streak_prev = target_streak) {
                if (streak > streak_prev) {
                    ...
                    if (streak_obj.lose_streak >= 2) {
                        streak_obj.amt := streak_obj.sum_amt * (0.15 + percs[1] + (0.1*(idx-2)))
        ...
        AmountOverride1_wl1_win7above() {
            ...
            HelperWinLossN(n) {
                ...
                lose_streak := this.wl_12[n].stats.lose_streak
                ...
                if (inst.streak > 0 and this.streak_prev[1] = n and inst.streak != this.streak_prev[1]) {
                    ...
                    this.wl_12[n].idx2 := 0
                    this.wl_12[n].stats.lose_streak := 0
                ...
                if (inst.streak < 0 and this.streak_prev[1] = n and inst.streak != this.streak_prev[1]) {
                    ...
                    this.wl_12[n].stats.lose_streak++
                    this.wl_12[n].stats.longest_lose_streak := max(this.wl_12[n].idx2, this.wl_12[n].stats.longest_lose_streak)
                    this.wl_12[n].idx1++
                }
                check_active := false
                for k, v in this.wl_12 {
        ...
        AmountOverride5_wl34() {
            ...
            if (this.max_diff.C >= 0 and this.F300.stateW = 0 and this.F300.stateL = 0) {
                this.F300.stateW := 1
                this.F300.stateL := -1
                this.F300.bal := this.max_diff.C
            }
            amts := [6+(0.5*this.F300.iter_lost5)]
            cent_amts := [0.10*(this.F300.iter_lost5+1)]
            Loop 100 {
                amts.Push(amts[-1]*2+3)
                cent_amts.Push(cent_amts[-1]*3+0.01)
            }
            percs := [0.3]
            Loop 100 {
        ...
        AmountOverride6_Lose7() {
            streak_obj := this.w5_l7[-7]
            streak := this.stats.streak_real
            streak_prev := this.streak_prev[1]
            if (streak <= -7 or streak_prev <= -7) {
                if (streak <= -7) {
                    target_streak := streak
                    streak_obj.lose_streak := abs(streak) - 7
                } else {
                    target_streak := streak_prev
                    streak_obj.lose_streak := 0
                }
        ...
        AmountOverride7_Win5_old() {
            ...
            if (streak > streak_prev and streak_prev >= 4) {
                ...
                if (streak_obj.pause5 = 1) {
                    streak_obj.pause5 := 0
                    idx := streak_obj.lose_streak
                    if (streak_obj.lose_streak < 1) {
                        idx := 1
                    }
                    streak_obj.amt := streak_obj.sum_amt * percs[idx]
                } else if (streak_obj.state_5lost = '5lost') {
                    streak_obj.lose_streak := 1 + 1
                    streak_obj.state_5lost := '5lostwon1'
                    streak_obj.amt := streak_obj.sum_amt * percs[streak_obj.lose_streak]
                } else if (streak_obj.state_5lost = '5lostwon1') {
                    streak_obj.lose_streak := 1
                    streak_obj.state_5lost := '5lostwon2'
            ...
            if (streak < streak_prev and streak_prev >= 5) {
                ...
                if (streak_obj.lose_streak = 7 or streak_obj.lose_streak = 5) {
                    this.wl_12[ 2].transfers++
                    sum_trf := streak_obj.sum_amt * 0.4
                    if streak_obj.lose_streak = 7 {
                        sum_trf := streak_obj.sum_amt * 0.8
                    }
                    this.wl_12[ 2].sum_amt += sum_trf/2
                    this.wl_12[-2].sum_amt += sum_trf/2
                    streak_obj.sum_amt -= sum_trf
                }
                if streak_obj.sum_amt >= 200 {
                ...
                if streak_obj.lose_streak > 0 {
                    streak_obj.amt := amts[streak_obj.lose_streak]
                }
                ; streak_obj.amt += cent_amts[streak_obj.lose_streak]
                if (streak_obj.pause5 = 1) {
                    streak_obj.amt := 1.1
                } else if (streak_obj.state_5lost = '5lostwon1') {
                    streak_obj.amt := streak_obj.sum_amt *percs[streak_obj.lose_streak]
                } else if (streak_obj.state_5lost = '5lostwon2') {
                    streak_obj.amt := cust_amt2won[Mod(streak_obj.lose_streak, cust_amt2won.Length) + 1]
                } else if (streak_obj.lose_streak >= 3) {
                    streak_obj.amt := streak_obj.sum_amt *percs[streak_obj.lose_streak - 2]
                    streak_obj.state_5lost := '5lost'
                }
            }
            ...
            if streak_prev >= 5 {
                streak_obj := this.win5andabove[streak_prev]
                if streak_obj.sum_amt >= 80 {
                    streak_obj.sum_over20 := 1
                }
                if streak_obj.sum_amt < 80 and streak_obj.sum_over20 = 1 {
                    streak_obj.sum_over20 := 0
                    streak_obj.lose_streak := 0
                    this.wl_12[ 2].sum_amt += streak_obj.sum_amt/2
                    this.wl_12[-2].sum_amt += streak_obj.sum_amt/2
                    streak_obj.sum_amt := 0
                    streak_obj.sum_over200 := 0
                    streak_obj.state_5lost := 0
            ...
            if (streak >= 5) {
                idx := Max(1, this.win5andabove[streak].lose_streak+1)
                if (streak_obj.ls_pause_temp > 0 and streak_obj.lose_streak = 0) {
                    idx += streak_obj.ls_pause_temp 
                }
        ...
        RankScenarios() {
            ...
            For key, value in this.stats.OwnProps() {
                ...
                sortableArray .= DecimalFormatter(value.win_rate, 1) ',' value.rank ',' key '`n'
            }
        ...
        TradeWin(streak_change:=true) {
            ...
            if this.stats.streak = -1 {
                this.qualifiers.loss_amount_modifier.amount_1 := (0.10*(this.max_diff.C)) / 0.92
            }
            if this.stats.streak = -2 {
                this.qualifiers.loss_amount_modifier.amount_2 := (0.10*(this.max_diff.C)) / 0.92
            }
            ...
            if this.stats.streak > 0 {
                ...
                if this.max_diff.C < 150 and this.qualifiers.custom_amount_modifier.state = 150
    ...
    SubReset1() {
        ...
        sum_trf := this.wl_12[ 1].sum_amt + this.wl_12[-1].sum_amt
        this.wl_12[ 1].sum_amt  := 0
        this.wl_12[-1].sum_amt := 0
        this.wl_12[ 2].sum_amt += sum_trf/2
        this.wl_12[-2].sum_amt += sum_trf/2
    ...
    StartLoop(*) {
        ...
        this.balance.side_high := this.balance.current
    ...
    UpdateLog() {
        ...
        if this.trade_opened[1] {
            ...
            countdown_close_str :=  this.executed_trades[1] ' [' this.stats.%this.executed_trades[1]%.win '|' this.stats.%this.executed_trades[1]%.draw '|' this.stats.%this.executed_trades[1]%.lose '] [' this.stats.%this.executed_trades[1]%.win_rate '% ' this.stats.%this.executed_trades[1]%.rank '/' 10 ']' ' (' DecimalFormatter(countdown_close)  ')'
        } else {
            countdown_close_str := ''
        }
        ...
        try
            str_c .= SubStr(this.crossovers_arr[-1].direction, 1, 1) DecimalFormatter((A_TickCount - this.crossovers_arr[-1].time)/1000, 1) ' | '
        ...
        str_d := ' (' Helper0811_4Loss.Get().level ') ' DecimalFormatter(this.amount)  '[' this.stats.streak_real ']'
        ...
        if (this.F300.stateW != 0 or this.F300.stateL != 0) {
            pref := 'w3=' this.wl34[3].losses ' w4=' this.wl34[4].losses ' l3=' this.wl34[-3].losses ' l4=' this.wl34[-4].losses
            str_win := 'W' this.F300.stateW
            str_lose := 'L' this.F300.stateL
            if this.F300.stateW > 1 {
                str_win := '0loss: ' this.wl34[this.F300.stateW].count_0loss '[max=' this.wl34[this.F300.stateW].max_count_0loss '] ' 'W' Abs(this.F300.stateW) '(' this.wl34[this.F300.stateW].lose_streak ' | sum: ' DecimalFormatter(this.wl34[this.F300.stateW].sum_amt)  ')'
            }
            if this.F300.stateL < -1 { 
                str_lose := '0loss: ' this.wl34[this.F300.stateL].count_0loss '[max=' this.wl34[this.F300.stateL].max_count_0loss '] ' 'L' Abs(this.F300.stateL) '(' this.wl34[this.F300.stateL].lose_streak ' | sum: ' DecimalFormatter(this.wl34[this.F300.stateL].sum_amt)  ')'
            }
            sub_str_d := '(' pref ') ' 'F300 ON [' DecimalFormatter(this.F300.sum_4lost)  '] (' str_win ' ' str_lose ')'
        ...
        for k, v in this.wl_12 {
            if (v.state = 'active') {
                ...
                _suffix2 := ' ' DecimalFormatter(cs_amts[Min(v.idx2, cs_amts.Length) or 1], 2) ') ' str_d
                if (k > 0) {
        ...
        str_h :=        '(W1: -' this.wl_12[ 1].lose_streak ' | 0loss=' this.wl_12[ 1].count_0loss '[max=' this.wl_12[ 1].max_count_0loss '] [sum=' DecimalFormatter(this.wl_12[ 1].sum_amt)  '])'
        str_h := str_h ' (L1: -' this.wl_12[-1].lose_streak ' | 0loss=' this.wl_12[-1].count_0loss '[max=' this.wl_12[-1].max_count_0loss '] [sum=' DecimalFormatter(this.wl_12[-1].sum_amt)  '])'
        ...
        win_rate := DecimalFormatter(this.stats.win_rate, 1) '%'
        if (this.flip_trade.state = 1) {
            win_rate := 'flip ' win_rate
        }
        if (this.stats.win + this.stats.loss) >= 15 {
            win_rate := win_rate ' [min:' DecimalFormatter(this.stats.min_win_rate, 1) '% | max:' DecimalFormatter(this.stats.max_win_rate, 1) '%]'
        }
        ...
        str_i := str_i ' (bet: ' DecimalFormatter(this.amount)  ')'
        if (streak = 5) {
            str_i := str_i ' sum=' DecimalFormatter(this.w5_l7[5].sum_amt)  ')'
        }
        ...
        loop {
            ...
            try {
                ...
                FileAppend(
                    ...
                    str.next_bal ',' 
                    this.last_trade ',' 
                    ' | ' this.payout '%=' DecimalFormatter(this.amount*1.92)  ' (' this.coin_name ')' ',' 
                    str_doublestats ',' 
                    str_ohlc ',' 
                    this.debug_str '`n',
                    this.log_file
    ...
    PSearch() {
        ...
        if this.ps.blue.state {
            ...
            this.coords.area.x2 := this.coords.area.x - 2
            ToolTip('(' A_Sec '.' A_MSec ')' this.debug_str '`nCurrent this.last_trade: ' this.last_trade '`nCurrent balance: ' DecimalFormatter(this.balance.current)   5, A_ScreenHeight*0.9, 11)
        } else {
            this.coords.area.x := max(this.coords.area.x - 1, 100)
            ...
            ; ToolTip('(' A_Sec '.' A_MSec ')' this.debug_str '`nCurrent this.last_trade: ' this.last_trade '`nCurrent balance: ' DecimalFormatter(balance.current)   5, 5, 11)
            return false
        }
        threshold := [10, 8]
        if this.ps.blue.state and this.ps.orange.state {
...
class Helper0811_4Loss {
    ...
    static Update(streak, streak_prev_list, max_bal_diff) {
        ...
        amts := [[1.40, 3.03, 6.98, 1], ; 16.18 changed to 1
                 [1,1,1,1],
