...
class TraderBot {
    ...
    CheckTradeClosed(just_check:=false) {
        ...
        if (streak > this.streak_prev[1]) {
            ...
            this.switch_win_loss[-1].sum_amt += sum_trf
            this.wl2_w5_l7[2].sum_amt += sum_trf
            this.wl2_w5_l7[-2].sum_amt += sum_trf
        ...
        CheckMaxDiff() {
            ...
            ResetLoseStreaks() {
                ...
                this.switch_win_loss[-1].lose_streak := 0
                this.wl2_w5_l7[2].lose_streak := 0
                this.wl2_w5_l7[-2].lose_streak := 0
                if (Abs(this.F300.stateW) >= 3) {
        ...
        HelperSumAmt(streak_obj, target_streak) {
            ...
            if (streak_prev = target_streak and streak < streak_prev) {
                streak_obj.sum_amt += this.amt_prev[1]
            }
            if (streak_prev = target_streak and streak > streak_prev) {
        ...
        HelperPause2_Loss5(streak_obj, target_streak) {
            ...
            if (streak > streak_prev and streak_prev = target_streak) {
                ...
                if (streak_obj.lose_streak = 0) {
                    streak_obj.next_bet_at_0 := streak_obj.last_bet_at_0 * (1.01 - this.reducer.val) + 1
                ...
                if (streak_obj.lose_streak = 0) {
                    amt_trf += 1/4
                    streak_obj.next_bet_at_0 := streak_obj.last_bet_at_0 * (0.75 - this.reducer.val)
        ...
        AmountOverride1_wl1_win7above() {
            ...
            HelperWinLossN(n) {
                idx := Min(this.switch_win_loss[n].idx1, this.switch_win_loss[n].amts.Length)
        ...
        AmountOverride8_22() {
            ...
            Helper(target_streak) {
                streak := this.stats.streak_real
                streak_prev := this.streak_prev[1]
                streak_obj := this.wl2_w5_l7[target_streak]
                idx := streak_obj.lose_streak > 0 ? 0 : Abs(streak_obj.lose_streak)
                HelperSumAmt(streak_obj, target_streak)
                if HelperPause1(streak_obj, target_streak, this.wl2_w5_l7[2], this.wl2_w5_l7[-2]) {
                    return
                }
                if HelperPause2_Loss5(streak_obj, target_streak) {
                    return
        ...
        AmountOverride7_Win5_old() {
            ...
            if streak_prev >= 5 {
                ...
                if streak_obj.sum_amt < 80 and streak_obj.sum_over20 = 1 {
                    streak_obj.sum_over20 := 0
                    streak_obj.lose_streak := 0
                    this.wl2_w5_l7[2].sum_amt += streak_obj.sum_amt/2
                    this.wl2_w5_l7[-2].sum_amt += streak_obj.sum_amt/2
                    streak_obj.sum_amt := 0
                    streak_obj.sum_over200 := 0
                    streak_obj.state_5lost := 0
        ...
        TradeWin(streak_change:=true) {
            ...
            if this.stats.max_bal_diff <= this.qualifiers.pause_temp1.reset_F {
            ...
            if this.amount >= 31 and this.stats.max_bal_diff <= 100 {
                ...
                this.qualifiers.pause_temp1.amount := this.stats.max_bal_diff
            }
    ...
    UpdateLog() {
        ...
        str_h := '(W1: 0loss=' this.switch_win_loss[1].count_0loss '[max=' this.switch_win_loss[1].max_count_0loss '] ' '-' this.switch_win_loss[1].lose_streak '[sum=' format('{:.2f}', this.switch_win_loss[1].sum_amt) '])'
        str_h := str_h ' (L1: 0loss=' this.switch_win_loss[-1].count_0loss '[max=' this.switch_win_loss[-1].max_count_0loss '] ' '-' this.switch_win_loss[-1].lose_streak '[sum=' format('{:.2f}', this.switch_win_loss[-1].sum_amt) '])'
        ...
        str_j := format('{:.2f}', this.stats.max_bal_diff) '[' format('{:.0f}', 75-this.reducer.val*100) '% | ' format('{:.0f}', 101-this.reducer.val*100) '%] (next=' format('{:.2f}', this.stats.next_max_bal_diff) ') (' this.qualifiers.streak_reset.count '|' this.qualifiers.streak_reset.count2 ')'
        ...
        str_m := '(L2: 0loss=' this.wl2_w5_l7[-2].count_0loss '[max=' this.wl2_w5_l7[-2].max_count_0loss ']) ' '(-' this.wl2_w5_l7[-2].lose_streak ' [wins=' this.wl2_w5_l7[-2].wins '|loss=' this.wl2_w5_l7[-2].losses ']) sum=' format('{:.2f}', this.wl2_w5_l7[-2].sum_amt)
        if (streak = -2) {
